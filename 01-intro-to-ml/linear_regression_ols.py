...
def get_inputs():   
    """Take dynamic n rows of (x, y) input from user."""
...
def compute_slope_intercept(x_vals, y_vals):    
    """Apply the OLS formulas to compute slope (m) and intercept (b)."""
    n = len(x_vals)

    # Accumulators (using loops, no built-in sum of products)
    sum_x = 0.0
    sum_y = 0.0
    sum_xy = 0.0
    sum_x2 = 0.0
...
def predict(m, b, x_new):   
    """Apply the linear equation ŷ = m*x + b."""
    return m * x_new + b


def compute_mse(x_vals, y_vals, m, b):    
...
def compute_r_squared(x_vals, y_vals, m, b):    
...
def main():
    ...
    # Step 2: Compute model parameters
    ...
    # Step 3: Evaluation on training data
